...
*/
		...
		$msg='<p>Congratulations '.$data['user_name'].'! <br>
			Order Accepted! Please wait some minutes to process.</p>';
			$edata['uemail']=$data['user_email'];
			$this->load->view('send_email_order_confirm',$edata);
...
$order_details = $this->db->select('*')->from('orders')->where('id',$id)->get()->result_array();

			$edata['uemail']=$order_details[0]['user_email'];
			$edata['uname']=$order_details[0]['user_name'];
			$this->load->view('send_email_order_confirm2',$edata);
...
$order_details = $this->db->select('*')->from('orders')->where('id',$id)->get()->result_array();
			...
			$edata['uemail']=$order_details[0]['user_email'];
			$edata['uname']=$order_details[0]['user_name'];
			$this->load->view('send_email_order_cancel',$edata);
